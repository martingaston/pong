...
function love.update(dt)
  ...
  if collides(ball, playerOne) then
    ...
    ball.dx = ball.dx * -1
    flux.to(ball, 0.1, { width = ball.width * 1.25, height = ball.height * 1.25 }):ease("circout"):after(ball, 0.25, { width = ball.width, height = ball.height })
    flux.to(divider, 0.1, { width = divider.width * 1.1, height = divider.height * 1.1 }):after(divider, 0.1, { width = divider.width, height = divider.height})
  end
  ...
  crt(function()
    ...
    background(function()
      ...
      for i = 5, screenY, 20 do
        love.graphics.rectangle('fill', screenX*.5 - 5, i, divider.width, divider.height)
      end
